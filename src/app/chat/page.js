...
async function fetchLearn(prompt) {
  const res = await fetch("/api/kyrai/learn", {
    method: "POST",
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === "learn") {
        ...
        const assessmentIntro = ["First, let's see what you know.", "I've prepared a quick quiz to see where you're at.", "Let's start with a short assessment.", "To begin, I'll ask you a few questions to gauge your understanding.", "Let's kick things off with a brief quiz."];
        setChatHistory((prev) => [
          ...prev,
          assessmentIntro[Math.floor(Math.random() * assessmentIntro.length)],
        ]);
      } else {
